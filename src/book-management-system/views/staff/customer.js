...
async function fetchCustomers(page = 1) {
  ...
  try {
    ...
    renderCustomerTable();
    renderCustomerPagination(data.totalPages, data.currentPage);
  } catch (error) {
...
document
  ...
  .addEventListener("click", async () => {
    ...
    if (name && birthDay && email) {
      ...
      try {
        const response = await fetch(
          `http://localhost:3000/profile/update/${currentEditId}`,
          {
            method: "PUT",
...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/profile/delete/${customerId}`,
        {
          method: "DELETE",
        }
      );

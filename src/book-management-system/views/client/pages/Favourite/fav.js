...
function displayFavItems(favData) {
  ...
  if (favData && favData.items && favData.items.length > 0) {
    favData.items.forEach((item) => {
      ...
      bookItem.innerHTML = `
          <div class="book-item" style="width: 20%; padding: 10px;">
              <img src="${thumbnail}" alt="Book cover of ${title}" style="width: 100%; height: auto;">
          </div>
          <div class="book-details" style="flex: 1; text-align: left; padding-left: 10px;">
              <div class="book-title" style="font-size: 18px; font-weight: 500; color: #333; margin-bottom: 10px;">
                  ${title}
              </div>
          </div>
          <div class="rating-title" style="flex: 1; font-size: 18px; text-align: center; padding: 10px;">
              ${starRating}
          </div>
          <div class="price-title" style="flex: 1; font-size: 18px; text-align: center; padding: 10px;">
              $${price.toFixed(2)}
          </div>
          <div class="status-title" style="flex: 1; text-align: center; padding: 10px;">
              <button onclick="removeFromFav('${
                item._id
              }')" style="background-color: #e74c3c; color: white; border: none; padding: 5px 10px; cursor: pointer;">Remove</button>
          </div>
        `;
